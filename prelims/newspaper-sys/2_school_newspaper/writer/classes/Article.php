...
class Article extends Database {
     ...
     */
    public function createArticle($title, $content, $imageName, $author_id, $category_id) {
        $sql = "INSERT INTO articles (title, content, imageName, author_id, category_id, is_active) 
                VALUES (?, ?, ?, ?, ?, 0)";
        return $this->executeNonQuery($sql, [$title, $content, $imageName, $author_id, $category_id]);
    }
    /**
    ...
    public function updateArticleCategory($id, $category_id) {
    $sql = "UPDATE articles SET category_id = ? WHERE article_id = ?";
    return $this->executeNonQuery($sql, [$category_id, $id]);
}
}
?>